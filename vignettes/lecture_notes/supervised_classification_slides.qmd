...
p <- ggplot(X, aes(x = x, y = y, color = group)) +
  geom_point(alpha = 0.7) +
  geom_contour(
    data = cbind(grid, z = as.numeric(pred$class == 2)),
    aes(x = x, y = y, z = z),
    breaks = 0.5,
...
train_pred <- simple_class(
  x = test_and_train$train$balance,
  boundary = 1400, above = "Yes", below = "No"
)
test_pred <- simple_class(
  x = test_and_train$test$balance,
  boundary = 1400, above = "Yes", below = "No"
)
```
...
train_default %>%
  ...
  stat_function(
    fun = function(x) {
      predict(
        model_glm, data.frame(balance = x),
        type = "response"
      )
    },
...
plot_clusters <- function(data, k) {
  ...
  p <- ggplot(data, aes(x = x, y = y, color = cluster)) +
    ...
    geom_segment(
      data = data, aes(
        xend = centers[kmeans_result$cluster, "x"],
...
ggplot(elbow_df, aes(x = k, y = wss)) +
  ...
  labs(
    title = "Scree Plot for k-means Clustering",
...
ggplot(data, aes(x = X, y = Y)) +
  ...
  labs(
    title = "Scatterplot with Regression Line and Projections onto Ŷ",
    x = "X Variable",
    y = "Y Variable"
  ) +
  theme_minimal()
...
ggplot(data, aes(x = X, y = Y)) +
  ...
  geom_segment(aes(x = X, y = Y, xend = proj_X, yend = proj_Y),
    color = "gray", linewidth = 0.5
  ) + # True orthogonal projection arrows
  geom_abline(
    slope = pc1_slope, intercept = pca$center[2] - pca$center[1] * pc1_slope,
    color = "dodgerblue", linewidth = 1
  ) + # Correct PCA axis
  coord_fixed() + # Ensures equal scaling of x and y axes
  labs(
    title = "Scatterplot with PCA Axis and True Orthogonal Projections",
    x = "X Variable",
    y = "Y Variable"
  ) +
  theme_minimal()
